...
int main(int argc, char*argv[]) {
    ...
    cout<<"quantity of uC:\Users\Tyler Purba\Documents\CompSCI\CIS-17A\Hmwk\Review HW1nique numbers"<<endl;
    cin>>arySize>>modNum;
...
int *mode(const int *array,int arySize){
    ...
    if(cnt>mx) mx=cnt; // catch tail run
    ...
    if(cnt==mx) nm++; // tail run again

   
    int *modeAry = new int[nm+2];
    modeAry[0]=nm;  // number of modes
    modeAry[1]=mx;  // frequency of each mode

   //fill modes into slots2
    int k=2; cnt=1;
    for(int i=1;i<arySize;i++){
        if(ary[i]==ary[i-1]){
            cnt++;
        } else {
            if(cnt==mx) modeAry[k++]=ary[i-1];
            cnt=1;
        }
    }
    if(cnt==mx) modeAry[k++]=ary[arySize-1]; // last run
