...
// Forward declarations so the Plyr constructor can call them
char **mkBrd(int, int);
void   clrBrd(char **, int, int, char);
...
void mkFlt(Ship *flt, int snum) {
    ...
    // if snum ever changes, make sure the rest are cleared
...
void updSt(Stat st[], int &sn, const char nm[], bool win) {
    ...
    if (idx == -1) {
        ...
        st[idx].setNm(nm);
        // pl and wn start at 0 from constructor
    }
